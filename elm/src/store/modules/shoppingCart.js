...
const mutations = {
  ...
  [type.ADD_TO_SHOPPINGCART](state, food) {
    const { foodName, shopId, price, shopName, foodType, foodId, foodNum } = food;
    // 判断购物中是否有此店铺
    if (Object.prototype.hasOwnProperty.call(state.shoppingCartProducts, shopId)) {
      let eachFlag = true;
      // 是否有相同的（包括同一商品类型不同）商品
      state.shoppingCartProducts[shopId].foodList.forEach((e) => {
        if (e.foodType === foodType && e.foodId === foodId) {
          eachFlag = false;
          e.foodNum += 1; // 如果有 就添加数量
      ...
      const data = { foodName, price, foodType, foodNum, foodId };
      tempObject[shopId].foodList.push(data); // 在选中商品数组当中添加当前商品
